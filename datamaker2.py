...
class Dataset_BOOLV(Dataset):
    ...
    def __read_data(self):
        ...
        b_t = 90
        df_data0 = df_raw[:365][self.features].reset_index(drop=True)[b_t:-15].reset_index(drop=True)
        df_data1 = df_raw[-365:][self.features].reset_index(drop=True)[b_t:-15].reset_index(drop=True)
        ...
        df_stamp0 = df_raw[['时间']][:365][b_t:-15]
        df_stamp0['date'] = pd.to_datetime(df_stamp0['时间'])
        data_stamp0 = time_features(pd.to_datetime(df_stamp0['date'].values), freq='d')
        data_stamp0 = data_stamp0.transpose(1, 0)
        data_stamp0 = np.hstack([data_stamp0, np.full((data_stamp0.shape[0], 1), -0.5)])

        df_stamp1 = df_raw[['时间']][-365:][b_t:-15]
        df_stamp1['date'] = pd.to_datetime(df_stamp1['时间'])
        data_stamp1 = time_features(pd.to_datetime(df_stamp1['date'].values), freq='d')
        data_stamp1 = data_stamp1.transpose(1, 0)
        data_stamp1 = np.hstack([data_stamp1, np.full((data_stamp1.shape[0], 1), 0.5)])

        self.data0 = data0
        self.data1 = data1
        self.data_stamp0 = data_stamp0
        self.data_stamp1 = data_stamp1
        self.range = (L, R)
    ...
    def __getitem__(self, index):
        ...
        near_stamp = self.data_stamp1[s_point+1-self.near_w: s_point+1]
        season_stamp = self.data_stamp1[s_point-30+1-self.season_w//2: s_point-30+1+self.season_w//2]
        trend_stamp = self.data_stamp0[s_point+1-(self.trend_w-self.predict_w): s_point+1+self.predict_w]
